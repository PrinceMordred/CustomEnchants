...
public class CustomEnchantsPlugin extends JavaPlugin implements Listener {
        ...
        for (Enchantment enchantment : Enchantments)
            if (!Arrays.stream(Enchantment.values()).toList().contains(enchantment)) //if not registered yet
                try {
                    ...
                    System.out.println("CustomEnchantments: Registered " + enchantment);
                } catch (Exception e) {
                    System.out.println("CustomEnchantments: Error: could not register " + enchantment);
                    e.printStackTrace();
                }
    }
    @EventHandler
    ...
    public void onEntityHit(EntityDamageByEntityEvent event){
        for (CustomEnchantment enchantment : Enchantments)
            enchantment.EventHandler(event);
    }
    @EventHandler
